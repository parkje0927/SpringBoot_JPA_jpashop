...
### 회원 조회 API
- 엔티티를 외부에 노출하지 마라
  + 실무에서는 member 엔티티의 데이터가 필요한 API 가 계속 증가하게 된다. 
  + 어떤 API 는 name 필드가 필요하지만, 어떤 API 는 name 필드가 필요 없을 수 있다. 
  + 결론적으로 엔티티 대신에 API 스펙에 맞는 별도의 DTO 를 노출해야 한다. 
